#!/bin/bash
#Script by Angela Shipman, v1-BRIDGES Nov 20, 2023
...
cores=`grep "\-np" $submit_script | awk '{print $3}'`
hrs=`grep "#SBATCH -t" $submit_script | awk '{print $3}'`
...
echo -e "\n${yellow}${bold}How many hours ${reset}${yellow}would you like to allocate for the ${bold}guest-only jobs?${reset}${yellow} Use the format HR:MM:SS. e.g. 2 hrs would be: 02:00:00"
read guest_hours
...
echo -e "\n${yellow}What ${bold}elements${reset}${yellow} is your host AND guest made of (INCLUDE ${bold}$guest${reset}${yellow} atoms)? ${bold}Type in elements separated by spaces,${reset}${yellow} and match the case sensitivity of the elements listed in your input script. e.g. H O C N Cu"
read atoms1_mof

total_jobs=$(( $num_guests + 2 ))

echo -e "${cyan}${bold}\n\n\n\n\nFINAL CHECK! ${reset}\n${cyan}Does this all sound right?\n\nAccording to your input, a total of ${bold}$total_jobs jobs ${reset}${cyan}will be submitted. \n\n${bold}HOST JOBS:${reset}${cyan} Host jobs will run for ${bold}$hrs hrs${reset}${cyan} with ${bold}$cores cores.${reset}${cyan} The host system is comprised of ${bold}$atoms1_mof elements ${reset}${cyan}for a total ${bold}host multiplicity of $host_multiplicity. \n\n${bold}GUEST JOBS:${reset}${cyan} There are ${bold}$num_guests $guest molecules${reset}${cyan} in your host system. The ${bold}$guest ${reset}${cyan}molecule is made of ${bold}$num_guest_atoms atoms${reset}${cyan}, which comprise of ${bold}$guest_elements elements${reset}${cyan}, for a total ${bold}guest multiplicity of $guest_multiplicity${reset}${cyan}. Guest jobs will be submitted with ${bold}$guest_cores cores${reset}${cyan} for ${bold}$guest_hours hours. \n\n${yellow}SUBMIT JOBS?${reset}"
...
cd all_real
sed -i "s/#SBATCH --job-name=.*/#SBATCH --job-name=\"all_real\"/g" $submit_script
...
sed -i "s/#SBATCH --job-name=.*/#SBATCH --job-name=\"host_real\"/g" $submit_script
sbatch $submit_script
cd ..
...
sed -i "s/#SBATCH -t.*/#SBATCH -t $guest_hours/g" guest.sh
sed -i "s/#SBATCH -ntasks-per-node=.*/#SBATCH -ntasks-per-node=$guest_cores/g" guest.sh
sed -i "s/mpirun -np.*/mpirun -np $guest_cores cp2k.popt -i *.inp -o runlog.log/g" guest.sh
...
do 
	...
	sed -i "s/#SBATCH --job-name=.*/#SBATCH --job-name=\"${i}${guest}\" /g" $submit_script
	sbatch $submit_script
	cd ..
