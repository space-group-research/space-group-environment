...
#SCRIPT BEGINS
#Check for runlog.log
echo -e "${cyan}Searching for ${bold}runlog.log${reset}${cyan}..."

if [ ! -f runlog.log ]  ;
then
	echo -e "${red}No ${bold}runlog.log${reset}${red} detected. Please ensure there is an output file in this directory.${reset}"
	exit
fi

#Check for Program
echo -e "${magenta}Detecting program...${reset}"
check=`head -n 1 runlog.log | awk '{print $1}'`

if [ "$check" = "DBCSR|" ] ;
then
	echo -e "${green}${bold}CP2K runlog${reset}${green} detected.${reset}"
	program=1 #CP2K
elif [ "$check" = "MPMC" ] ;
then
	echo -e "${green}${bold}MPMC runlog${reset}${green} detected.${reset}"
	program=2 #MPMC
else
...
then
	...
	#necessary axes:
	#1step number: grep "Completed step" runlog.log | awk '{print $4}' | awk -F"/" '{print $1}'
	#2BF: grep "BF" runlog.log | awk '{print $4}'
	#3AR (fraction of moves accepted): grep "AR" runlog.log | awk '{print $4}'
	#4potential energy: grep "potential energy" runlog.log | awk '{print $5}'
	#5electrostatic energy: grep "electrostatic energy" runlog.log | awk '{print $5}'
	#REMOVE 2 FIRST LINES 6repulsion/dispersion energy: grep "repulsion/dispersion energy" runlog.log | awk '{print $5}'
	#REMOVE 2 FIRST LINES 7N: grep "N =" runlog.log | awk '{print $4}'
	#REMOVE 2 FIRST LINES 8density: grep "density" runlog.log | awk '{print $4}'
	#REMOVE 2 FIRST LINES 9wt %: grep "wt % =" runlog.log | awk '{print $5}'
	#REMOVE 2 FIRST LINES 10wt % (Mommaed edowdy): grep "wt % (ME) =" runlog.log | awk '{print $6}'
	#11qst (kJ/mol): grep "qst" runlog.log | awk '{print $4}'
	#12energy/particle: grep "Energy/particle" runlog.log | awk '{print $4}'
	#13compressibility: grep "compressibility" runlog.log | awk '{print $4}'
	#14bulk modulus: grep "bulk modulus" runlog.log | awk '{print $5}'
	#15heat capacity: grep "heat capacity" runlog.log | awk '{print $5}'
	#REMOVE 2 FIRST LINES 16para spin ratio: grep "para spin ratio" runlog.log | awk '{print $6}'

	#x-axis
	if [ $x_axis == 1 ] ;
                        then
                                grep "Completed step" runlog.log | awk '{print $4}' | awk -F"/" '{print $1}' > x_axis.txt
				x_label="Step Number"

                        elif [ $x_axis == 2 ] ;
                        then
                                grep "BF" runlog.log | tail -n +2 | awk '{print $4}' > x_axis.txt
				x_label="Boltzmann Factor"

                        elif [ $x_axis == 3 ] ;
                        then
                                grep "AR" runlog.log | tail -n +2 | awk '{print $4}' > x_axis.txt
				x_label="Acceptance Ratio"

                        elif [ $x_axis == 4 ] ;
                        then
                                grep "potential energy" runlog.log | tail -n +2 | awk '{print $5}' > x_axis.txt
				x_label="Potential Energy [K]"

                        elif [ $x_axis == 5 ] ;
                        then
                                grep "electrostatic energy" runlog.log | tail -n +2 | awk '{print $5}' > x_axis.txt
				x_label="Electrostatic Energy [K]"

                        elif [ $x_axis == 6 ] ;
                        then
                                grep "repulsion/dispersion energy" runlog.log | tail -n +3 | awk '{print $5}' > x_axis.txt
				x_label="Repulsion/Dispersion Energy [K]"

                        elif [ $x_axis == 7 ] ;
                        then
                                grep "N =" runlog.log | tail -n +3 | awk '{print $4}' > x_axis.txt
				x_label="N [# sorbed molecules in system]"

                        elif [ $x_axis == 8 ] ;
                        then
				grep "density" runlog.log | tail -n +3 | awk '{print $4}' > x_axis.txt
                                x_label="Density of Sorbed Molecules [g/cm^3]"

                        elif [ $x_axis == 9 ] ;
                        then
				grep "wt % =" runlog.log | tail -n +3 | awk '{print $5}' > x_axis.txt
                                x_label="wt %"

                        elif [ $x_axis == 10 ] ;
                        then
				grep "wt % (ME) =" runlog.log | tail -n +3 | awk '{print $6}' > x_axis.txt
				x_label="wt % (ME)"

			elif [ $x_axis == 11 ] ;
                        then
				grep "qst" runlog.log | tail -n +2 | awk '{print $4}' > x_axis.txt
				x_label="qst [kJ/mol]"

			elif [ $x_axis == 12 ] ;
                        then
                                grep "Energy/particle" runlog.log | tail -n +2 | awk '{print $4}' > x_axis.txt
				x_label="Energy/particle [kJ/mol]"

			elif [ $x_axis == 13 ] ;
                        then
                                grep "compressibility" runlog.log | tail -n +2 | awk '{print $4}' > x_axis.txt
				x_label="Compressibility [atm^(-1)]"

			elif [ $x_axis == 14 ] ;
                        then
                                grep "bulk modulus" runlog.log | tail -n +2 | awk '{print $5}' > x_axis.txt
				x_label="Bulk Modulus [GPa]"

			elif [ $x_axis == 15 ] ;
                        then
                                grep "heat capacity" runlog.log | tail -n +2 | awk '{print $5}' > x_axis.txt
				x_label="Heat Capacity [kJ/mol K]"

			elif [ $x_axis == 16 ] ;
                        then
				grep "para spin ratio" runlog.log | tail -n +3 | awk '{print $6}' > x_axis.txt
        ...
        if [ $y_axis == 1 ] ;
                        then
                                grep "Completed step" runlog.log | awk '{print $4}' | awk -F"/" '{print $1}' > y_axis.txt
                                y_label="Step Number"

                        elif [ $y_axis == 2 ] ;
                        then
                                grep "BF" runlog.log | tail -n +2 | awk '{print $4}' > y_axis.txt
                                y_label="Boltzmann Factor"

                        elif [ $y_axis == 3 ] ;
                        then
                                grep "AR" runlog.log | tail -n +2 | awk '{print $4}' > y_axis.txt
                                y_label="Acceptance Ratio"

                        elif [ $y_axis == 4 ] ;
                        then
                                grep "potential energy" runlog.log | tail -n +2 | awk '{print $5}' > y_axis.txt
                                y_label="Potential Energy [K]"

                        elif [ $y_axis == 5 ] ;
                        then
                                grep "electrostatic energy" runlog.log | tail -n +2 | awk '{print $5}' > y_axis.txt
                                y_label="Electrostatic Energy [K]"

                        elif [ $y_axis == 6 ] ;
                        then
                                grep "repulsion/dispersion energy" runlog.log | tail -n +3 | awk '{print $5}' > y_axis.txt
                                y_label="Repulsion/Dispersion Energy [K]"

                        elif [ $y_axis == 7 ] ;
                        then
                                grep "N =" runlog.log | tail -n +3 | awk '{print $4}' > y_axis.txt
                                y_label="N [# sorbed molecules in system]"

                        elif [ $y_axis == 8 ] ;
                        then
                                grep "density" runlog.log | tail -n +3 | awk '{print $4}' > y_axis.txt
                                y_label="Density of Sorbed Molecules [g/cm^3]"

                        elif [ $y_axis == 9 ] ;
                        then
                                grep "wt % =" runlog.log | tail -n +3 | awk '{print $5}' > y_axis.txt
                                y_label="wt %"

                        elif [ $y_axis == 10 ] ;
                        then
                                grep "wt % (ME) =" runlog.log | tail -n +3 | awk '{print $6}' > y_axis.txt
                                y_label="wt % (ME)"

                        elif [ $y_axis == 11 ] ;
                        then
                                grep "qst" runlog.log | tail -n +2 | awk '{print $4}' > y_axis.txt
                                y_label="qst [kJ/mol]"

                        elif [ $y_axis == 12 ] ;
                        then
                                grep "Energy/particle" runlog.log | tail -n +2 | awk '{print $4}' > y_axis.txt
                                y_label="Energy/particle [kJ/mol]"

                        elif [ $y_axis == 13 ] ;
                        then
                                grep "compressibility" runlog.log | tail -n +2 | awk '{print $4}' > y_axis.txt
                                y_label="Compressibility [atm^(-1)]"

                        elif [ $y_axis == 14 ] ;
                        then
                                grep "bulk modulus" runlog.log | tail -n +2 | awk '{print $5}' > y_axis.txt
                                y_label="Bulk Modulus [GPa]"

                        elif [ $y_axis == 15 ] ;
                        then
                                grep "heat capacity" runlog.log | tail -n +2 | awk '{print $5}' > y_axis.txt
				y_label="Heat Capacity [kJ/mol K]"

                        elif [ $y_axis == 16 ] ;
                        then
                                grep "para spin ratio" runlog.log | tail -n +3 | awk '{print $6}' > y_axis.txt
